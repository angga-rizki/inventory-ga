<?php
$titleModalTambah = 'Tambah Barang Keluar';
$titleModalEdit = 'Edit Barang Keluar';
$linkSaveData = base_url('barang_keluar/save_barang_keluar');
$linkDeleteData = base_url('barang_keluar/delete_barang_keluar');
?>
                        ...
                        <div class="data-input-barang">
                            <div class="d-flex flex-row alig align-items-end">
                                <div class="form-group col p-0 mr-2">
                                    ...
                                    <select class="form-control input-id-barang" id="inputIdNamaBarang-0" name="inputIdNamaBarang[]" required="">
                                        <option value="" selected>Pilih barang...</option>
                                    </select>
                                </div>

                                <div class="form-group col-4 p-0">
                                    <label for="inputQuantityOut-0">Quantity Out</label>
                                    <input type="number" class="form-control input-quantity-barang" id="inputQuantityOut-0" name="inputQuantityOut[]" min="0"  required="">
                                </div>           
                    ...
                    <div id="cardTabel" class="card">
                        <div class="card-body">
                            <table id="datatable" class="table table-bordered table-striped table-sm" style="width: 100%;">
                                ...
                                <tbody>
                                    <?php $no = 1; ?>
                                    <?php foreach ($listBarangKeluar as $list): ?>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#bootstrapModal').on('show.bs.modal', function (event) {
            ...
            isiOptionNamaBarang = '';
            $.ajax({
                url: 'barang_keluar/ajax_load_list_select_nama_barang',
            }).done(function (data) {
                ...
                for (var i = 0; i < listNamaBarang.length; i++) {
                    ...
                    var valueSisa = listNamaBarang[i]['sisa'];

                    // hash map
                    sisaBarangHashMap[idBarang] = valueSisa;

                    isiOptionNamaBarang += '<option value="' + idBarang + '">' + listNamaBarang[i]['nama_barang'] + '</option>';
                }
                ...
                if (aksi == 'edit') {
                    ...
                    var idNamaBarang = barisTabelButtonAksi.data('idbarang');
                    var tanggal = barisTabelButtonAksi[0].cells[2].innerHTML;
                    var quantityOut = barisTabelButtonAksi[0].cells[4].innerHTML;

                    // data untuk dikirim
                    $('#id').val(id);
                    $('#inputIdNamaBarang-0').val(idNamaBarang);
                    $('#inputTanggal').val(tanggal);
                    $('#inputQuantityOut-0').val(quantityOut);
                    formatTanggal = $('#inputTanggal').val() + ' ' + time;
                    $('#inputFormatTanggal').val(formatTanggal);

                    // set max value inputQuantityOut
                    var sisaBarang = parseInt(sisaBarangHashMap[$('#inputIdNamaBarang-0').val()]);
                    $('#inputQuantityOut-0').attr('max', ((sisaBarang) + parseInt(quantityOut)));
                }
            });
            ...
            $('#dataBarang').on('change', '.input-id-barang', function () {
                var dataTambahanIni = $(this).closest('.data-input-barang');

                // set max value inputQuantityOut sesuai dengan sisa barang
                $(dataTambahanIni).find('.input-quantity-barang').val('');
                $(dataTambahanIni).find('.input-quantity-barang').attr('max', sisaBarangHashMap[$(this).val()]);
            });
        ...
        $('#tambahDataBarangTambahanButton').on('click', function () {
            $('#dataBarang').append(
                    '<div class="data-input-barang data-tambahan d-flex flex-row alig align-items-end">' +
                    '<div class="form-group flex-column col p-0 mr-2">' +
                    '<label for="inputIdNamaBarang-' + idHtml + '">Nama Barang</label>' +
                    '<select class="form-control input-id-barang" id="inputIdNamaBarang-' + idHtml + '" name="inputIdNamaBarang[]" required="">' +
                    '<option value="" selected>Pilih barang...</option>' +
                    '</select>' +
                    '</div>' +
                    '<div class="form-group flex-column col-4 p-0">' +
                    '<label for="inputQuantityOut-' + idHtml + '">Quantity In</label>' +
                    '<input type="number" class="form-control input-quantity-barang" id="inputQuantityOut-' + idHtml + '" name="inputQuantityOut[]" min="0"  required="">' +
                    '</div>' +
